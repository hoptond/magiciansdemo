						...
						case (4):
							{
								if (canFlee)
								{
									...
									game.Audio.PlaySound("ButtonClick", true); game.Audio.PlaySound("RunAway", true); EndState = BattleEnd.Fled; currentState = State.BattleResult; game.EndBattle(EndState); displayBattleUI = false; game.Audio.SetMusic(ReturnMusic); game.enemySpawnManager.FlagSpawn(game.TemporaryMap.Filename, game.BattleGroupNumber);
								}
								break;
							}
